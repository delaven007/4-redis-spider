...
```python
1、Redis基于内存,速度快
#scrapy每一个爬取的数据，对应一个指纹。没下载一次数据，scrapy不会记录下载数据，加载完毕后下载记录会丢失掉。
...
```python
1、ROBOTSTXT_OBEY = False  取消注释，禁用cookie
2、LOG_LEVEL = 'WARNING'
...
# 原理
通过扫描等光学输入方式将各种票据、报刊、书籍、文稿及其它印刷品的文字转化为图像信息，再利用**文字识别技术**将图像信息转化为电子文本
```
...
```python
1、获取验证码图片 - 到本地
2、使用PIL库打开图片 	-img=image.open()	
3、使用pytesseract将图片中验证码识别并转为字符串	pytesseract.image_to_string(img)
