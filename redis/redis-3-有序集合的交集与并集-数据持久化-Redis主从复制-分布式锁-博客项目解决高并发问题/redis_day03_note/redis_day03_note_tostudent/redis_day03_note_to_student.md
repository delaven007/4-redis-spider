...
```python
rlist=r.revrange('rangking',0,2,withscores=True)
n=1
for name in rlist:
...
问题：如何获取三款手机的销量排名？
ZUNIONSTORE mobile-001:003 mobile-001 mobile-002 mobile-003 # 可否？否，默认SUM
# 正确
方法1:zrevrange mobile-003
...
OK
# 特点
1、执行SAVE命令过程中，redis服务器将被**阻塞**(阻塞请求)，无法处理客户端发送的命令请求，在SAVE命令执行完毕后，服务器才会重新开始处理客户端发送的命令请求
...
219行: save 300 10
220行: save 60 10000
  1、只要三个条件中的**任意**一个被满足时，服务器就会自动执行BGSAVE
...
1 既有dump.rdb,又有appendonly.aof，回复时找谁?
...
# aof持久化开启
9、appendonly yes
10、apendfilename 'appendonly.aof'
# aof持久化策略
...
2、master会一直将自己的数据更新同步给slaves，保持**主从同步**
...
2. 创建配置文件   sentinel.conf
	port 6379
	sentinel monitor 名字 IP port 投票数(3->2,5->3)
3.启动sentinel开始监控
